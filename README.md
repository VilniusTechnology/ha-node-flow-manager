...
- **Node-RED**: Accessible via network (e.g., addon or separate container).
    - If using **Node-RED Addon** in HA: Ensure the "credential_secret" is set if you want to use authentication, or simpler, use the internal URL `http://a0d7b954-nodered:1880` (hostname may vary).
...
| :--- | :--- | :--- |
| **Host** | The hostname or IP address of your Node-RED instance. | `192.168.1.100` or `localhost` |
...
| **Public URL** | (Optional) External URL to link to Node-RED from HA. | `https://node-red.example.com` |
